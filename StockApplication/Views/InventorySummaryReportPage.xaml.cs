...
public partial class InventorySummaryReportPage : ContentPage
{
    ...
    public InventorySummaryReportPage(StockRepository stockRepository, Repository<Supplier> supplierRepository)
    {
        InitializeComponent();
        ...
        StockItemsCollection.ItemsSource = _stockItems;
        ...
        ApplyFiltersButton.Clicked += OnApplyFiltersClicked;
    ...
    private async void LoadReportData()
    {
        try
        {
            ...
            // Filter by date if needed (using UpdatedAt)
            var filteredStocks = stocks.Where(s => 
                s.UpdatedAt >= StartDatePicker.Date && 
                s.UpdatedAt <= EndDatePicker.Date.AddDays(1).AddSeconds(-1)).ToList();
            
            // Calculate summary statistics
            double totalItems = filteredStocks.Count;
            double totalQuantity = filteredStocks.Sum(s => s.Quantity);
            double averageQuantity = totalItems > 0 ? totalQuantity / totalItems : 0;
            
            // Update summary labels
            MainThread.BeginInvokeOnMainThread(() => {
                TotalItemsLabel.Text = totalItems.ToString();
                TotalQuantityLabel.Text = totalQuantity.ToString("F2");
                AverageQuantityLabel.Text = averageQuantity.ToString("F2");
            });
        ...
        {
            await DisplayAlert("Error", $"Failed to load report data: {ex.Message}", "OK");
        }
        finally
        {
            IsBusy = false;
    ...
    private async void OnExportCsvClicked(object sender, EventArgs e)
    {
        try
        {
            ...
            string fileName = $"Inventory_Summary_{DateTime.Now:yyyyMMdd_HHmmss}.csv";
    ...
    private async void OnExportPdfClicked(object sender, EventArgs e)
    {
        try
        {
            ...
            string fileName = $"Inventory_Summary_{DateTime.Now:yyyyMMdd_HHmmss}.pdf";
            string filePath = Path.Combine(FileSystem.CacheDirectory, fileName);
            ...
            using (var fileStream = new FileStream(filePath, FileMode.Create))
            {
                ...
                // Add title
                var titleFont = FontFactory.GetFont(FontFactory.HELVETICA_BOLD, 18, new BaseColor(76, 175, 80)); // Green color
                var title = new Paragraph("Inventory Summary Report", titleFont);
                ...
                var dateRange = new Paragraph(
                    $"Report Period: {StartDatePicker.Date:MM/dd/yyyy} to {EndDatePicker.Date:MM/dd/yyyy}", dateFont);
                document.Add(dateRange);
                document.Add(new Paragraph(" ")); // Add spacing
                
                // Add summary statistics
                var summaryFont = FontFactory.GetFont(FontFactory.HELVETICA_BOLD, 14, new BaseColor(76, 175, 80)); // Green color
                ...
                string[] summaryHeaders = { "Total Items", "Total Quantity", "Average Quantity" };
                foreach (var header in summaryHeaders)
                {
                    var headerCell = new PdfPCell(new Phrase(header, headerFont));
                    headerCell.BackgroundColor = new BaseColor(76, 175, 80); // Green color
                ...
                var totalItemsCell = new PdfPCell(new Phrase(TotalItemsLabel.Text, valueFont));
                totalItemsCell.HorizontalAlignment = PdfElement.ALIGN_CENTER;
                totalItemsCell.VerticalAlignment = PdfElement.ALIGN_MIDDLE;
                totalItemsCell.Padding = 8;
                summaryTable.AddCell(totalItemsCell);
                
                var totalQuantityCell = new PdfPCell(new Phrase(TotalQuantityLabel.Text, valueFont));
                totalQuantityCell.HorizontalAlignment = PdfElement.ALIGN_CENTER;
                totalQuantityCell.VerticalAlignment = PdfElement.ALIGN_MIDDLE;
                totalQuantityCell.Padding = 8;
                summaryTable.AddCell(totalQuantityCell);
                
                var avgQuantityCell = new PdfPCell(new Phrase(AverageQuantityLabel.Text, valueFont));
                avgQuantityCell.HorizontalAlignment = PdfElement.ALIGN_CENTER;
                avgQuantityCell.VerticalAlignment = PdfElement.ALIGN_MIDDLE;
                avgQuantityCell.Padding = 8;
                summaryTable.AddCell(avgQuantityCell);
                
                document.Add(summaryTable);
                document.Add(new Paragraph(" ")); // Add spacing
                
                // Add inventory details header
                var inventoryFont = FontFactory.GetFont(FontFactory.HELVETICA_BOLD, 14, new BaseColor(76, 175, 80)); // Green color
                var inventoryHeader = new Paragraph("Inventory Details", inventoryFont);
                ...
                foreach (var item in _stockItems)
                {
                    ...
                    // Quantity
                    var qtyCell = new PdfPCell(new Phrase(item.Quantity.ToString("F2"), cellFont));
            ...
            switch (action)
            {
                case "Share File":
                    ...
                    await Share.RequestAsync(new ShareFileRequest
                    {
                        Title = "Export Inventory Summary PDF",
                        File = new ShareFile(filePath)
                    });
                    break;
...
// View model class for stock items
